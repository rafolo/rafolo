...
class apt_get_update {
  exec { 'apt-get -y update': unless => "test -e ${home}/.rvm" }
}
...
package { ['sqlite3', 'libsqlite3-dev']: ensure => installed; }
...
class install_mysql {
  ...
  database_grant { [
    'rails@localhost/activerecord_unittest',
    'rails@localhost/activerecord_unittest2',
    'rails@localhost/inexistent_activerecord_unittest']:
    privileges => ['all'],
...
package { ['libxml2', 'libxml2-dev', 'libxslt1-dev']: ensure => installed }
